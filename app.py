...
async def scrape_one_with_context(browser, asin):
    ...
    try:
        ...
        for sel in ["#corePrice_feature_div .a-price .a-offscreen", "#corePriceDisplay_desktop_feature_div .a-price .a-offscreen", ".priceToPay .a-offscreen", ".a-price .a-offscreen"]:
            try:
...
def index():
    ...
    # Fetch user data to pass to template
    cur.execute("SELECT email, is_approved FROM users WHERE id=%s", (uid,))
    user_row = cur.fetchone()
    if not user_row or not user_row.get("is_approved"):
        conn.close(); return redirect(url_for("waitlist_page"))
...
def admin_portal():
    ...
    if tab == "dashboard":
        ...
        q = "SELECT i.*, u.email AS user_email, (SELECT ph.item_name FROM price_history ph WHERE ph.item_id=i.id ORDER BY ph.ts DESC LIMIT 1) AS latest_name FROM items i JOIN users u ON u.id=i.user_id"
