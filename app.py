...
def write_history(item_id, data):
    if not data.get("price_value"):
        if data.get("coupon_text"):
            ...
            if latest_row:
                cur.execute("UPDATE price_history SET coupon_text=%s WHERE id=%s", (data["coupon_text"], latest_row["id"]))
            conn.commit()
            conn.close()
        return
    ...
    if insert:
        cur.execute("""
            INSERT INTO price_history(item_id, ts, item_name, price_text, price_value, coupon_text, error) 
            VALUES(%s, %s, %s, %s, %s, %s, %s)
        """, (item_id, data.get("timestamp"), data.get("item_name"), data.get("price_text"), data.get("price_value"), data.get("coupon_text"), data.get("error")))
        conn.commit()
    conn.close()
...
def index():
    ...
    for it in items:
        ...
        enriched.append({
            "id": it["id"], "asin": it["asin"], "url": it["url"], "target_price": it.get("target_price"), 
            "latest_name": latest.get("item_name"), "latest_price_text": latest.get("price_text"), 
            "latest_price_value": latest.get("price_value"), "coupon_text": latest.get("coupon_text"), 
            "latest_ts": latest.get("ts"), "is_lowest": is_lowest
        })
    conn.close()
...
def admin_portal():
    ...
    if tab == "dashboard":
        ...
        """
        if f:
            cur.execute(q + " WHERE u.id=%s ORDER BY i.created_at DESC", (f,))
        else:
            cur.execute(q + " ORDER BY i.created_at DESC")
