...
def init_db():
    ...
    # 4. System Settings Table (For Intervals & Announcements)
    cur.execute("""
    ...
    try:
        ...
        cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS item_limit INTEGER DEFAULT 20;")
        ...
        # Ensure existing users aren't locked out
...
async def scrape_one_with_context(browser, asin):
    ...
    data = {
        ...
        "coupon_text": None, "discount_percent": None, "rating": None, "reviews_count": None
    }
...
def write_history(item_id, data):
    ...
    if insert:
        cur.execute("""
            INSERT INTO price_history(item_id, ts, item_name, price_text, price_value, coupon_text, rating, reviews_count, discount_percent, error) 
            VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (item_id, data["timestamp"], data["item_name"], data["price_text"], data["price_value"], data["coupon_text"], data["rating"], data["reviews_count"], data["discount_percent"], data["error"]))
        conn.commit()
    conn.close()
...
def run_global_scrape():
    ...
    for asin, data in results.items():
        ...
        prev = cur.fetchone()
        prev_c = prev['coupon_text'] if prev else None
        ...
        for item in rows:
            ...
            if data['coupon_text'] and data['coupon_text'] != prev_c:
                send_alert_email(item['email'], asin, data['item_name'], data['price_value'], is_coupon=True)
    conn.close()
